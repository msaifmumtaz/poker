...
RANKS = "A23456789TJQK"
SUITS = "cdhs"
STRAIGHTS = [list(f"{RANKS}A")[ind : ind + 5] for ind in range(10)]
...
class HandAnalyzer(object):
    ...
    def __init__(self, hand, payouts=None):
        self.__specials = (
            ""  # special 4kinds ranks see: DiscardValue._four_kind_special()
        )
        if payouts is None:
        ...
        for ind in range(0, 10, 2):
            self.hand.append(
                hand[ind : ind + 1].upper() + hand[ind + 1 : ind + 2].lower()
            )
    ...
    def analyze(self, return_full_analysis=True, return_bestdisc_cnts=True):
        ...
        for hold_l in product([True, False], repeat=5):
            ...
            hand = "".join(
                [card if held else "XX" for card, held in zip(self.hand, hold_l)]
            )
    ...
    def pay_current_hand(self):
        ...
        strt_set = {"".join(x) for x in STRAIGHTS}
...
class DiscardValue(object):
    ...
    def __init__(self, held_d=None, hand_str=None, hold_str=None, specials=None):
        if held_d is not None:
            ...
            hold_l = [
                hold_str[ind : ind + 2].upper() != "XX" for ind in range(0, 10, 2)
            ]
    ...
    def royal_flush(self):
        ...
        return 1 if len(set(self.held_s)) == 1 else 4 - len(discarded_royal_suits)
    ...
    def flush(self):
        ...
        if num_suits == 1:
            ...
            for udc in undrawable_suit_cnt.values():
    ...
    def pair_jqka(self):
        ...
        if not self.held_r_cnts:
            return self._draw_for_ranks(gsize=2, cnt_held_only=False, pairing_jqka=True)
            ...
            two_pair_bool = (len(self.held_r_cnts) > 1) and (
                self.held_r_cnts[1][1] == 2
            )
        ...
        # most common card is a singleton
            ...
            if self.draw_cnt == 4:
                ...
                for avail, rcnt in self.nonheld_rank_grps.items():
                    ...
                    new_nhrg[avail] -= 1
                    kick_ways = self._count_ways2kick(
                        nonheld_rank_grps=new_nhrg, num_kickers=1
                    )
                ...
                for havail, hrcnt in held_r_avail_grps.items():
                    ...
                    # then draw a pair
                    drawways = self._draw_for_ranks(
                        gsize=2, draw_cnt=2, draw_only=True, second_pair=False
                    )
                ...
                return self._draw_2pair(nonheld_rank_grps=held_r_avail_grps, draw_cnt=2)
    ...
    def three_kind(self):
        ...
        if self.held_r_cnts == [] or self.held_r_cnts[0][1] == 1:
            return self._draw_for_ranks(gsize=3, cnt_held_only=False)
            ...
            if len(self.held_r_cnts) != 2 or self.held_r_cnts[1][1] != 1:
                return drawp
    ...
    def full_house(self):
        held_r_avail = {r: self.__draws[r] for r in self.held_r}
        held_r_avail_grps = Counter(held_r_avail.values())
        ...
        if not self.held_r_cnts:
            ...
            if self.draw_cnt == 4:
                ...
                for heldneed, drawneed in [[1, 3], [2, 2]]:
                    ...
                    draw_grp = self._draw_for_ranks(
                        gsize=drawneed, draw_cnt=drawneed, draw_only=True
                    )
            ...
            if self.draw_cnt in [3, 2]:
                ...
                if self.draw_cnt == 3:
                    ...
                    ways_cnt += self._draw_for_ranks(
                        gsize=3, draw_cnt=3, draw_only=True
                    )
                    # draw pair to go with tripup pair
                    draws = self._draw_for_ranks(gsize=2, draw_cnt=2, draw_only=True)
                elif self.draw_cnt == 2:
                ...
                # check for holding two pair
                return sum(held_r_avail.values()) if self.held_r_cnts[1][1] == 2 else 0
            else:
                return 0
        elif self.held_r_cnts[0][1] == 3:
            # draw pair with trips,
            if self.draw_cnt == 2:
                return self._draw_for_ranks(gsize=2, draw_cnt=2, draw_only=True)
            elif self.draw_cnt == 1:
    ...
    def four_kind(self, specials=None):
        ...
        return draw if specials is None else draw - self._four_kind_special(specials)
        ...
        if not cnt_held_only:
            for avail, rcnt in draw_grp_iter:
                ...
                if kickers > 0:
                    ...
                    new_nhrg[avail] -= 1
                    kick_ways = self._count_ways2kick(
                        nonheld_rank_grps=new_nhrg, num_kickers=kickers
                    )
                else:
                    kick_ways = 1
                ways_cnt += rways * kick_ways
            ...
            for r, hcnt in Counter(self.held_r).items():
                ...
                if kickers > 0:
                    if not second_pair:
                        ...
                        sp_nhrg = nonheld_rank_grps_mod - Counter(
                            {nonheld_ranks_mod[r]: 1}
                        )
                        kick_ways = self._count_ways2kick(
                            nonheld_rank_grps=sp_nhrg, num_kickers=kickers
                        )
                else:
                    kick_ways = 1
    ...
    def _count_ways2kick(self, nonheld_rank_grps=None, num_kickers=1):
        ...
        kick_cnt = 0
        for suit_cnt_tup in combinations_with_replacement(
            nonheld_rank_grps, num_kickers
        ):
            multiplier = 1
...
if __name__ == "__main__":
    print(
        HandAnalyzer("3s8dth9SAC").analyze(
