...
def detect(file):
    try:
        ...
        valueSize, baseline = cv2.getTextSize(
            "123456789VDRA", cv2.FONT_HERSHEY_SIMPLEX, 1, 2
        )
        ...
        cnts_val, hierarchy = cv2.findContours(
            values_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        cnts_sym, hierarchy = cv2.findContours(
            symbols_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        for c in cnts:
            if cv2.contourArea(c) > 500:
                ...
                if len(approx) == 4:
                    ...
                    pts1 = np.array(
                        [pts1_cw[0], pts1_cw[1], pts1_cw[3], pts1_cw[2]],
                        dtype="float32",
                    )
                    pts2 = np.array(
                    ...
                    if width > height:  # landscape
                        flat_card = cv2.rotate(
                            flat_card, cv2.ROTATE_90_COUNTERCLOCKWISE
                        )
                    # thresh the extracted card
                    ret, thresh = cv2.threshold(
                        flat_card, 140, 255, cv2.THRESH_BINARY_INV
                    )
                    ...
                    cur_cnts, hierarchy = cv2.findContours(
        ...
        # Print the output
        output_string = "".join(
            card_to_string(card_info, card_symbols) for card_info in output
        )
